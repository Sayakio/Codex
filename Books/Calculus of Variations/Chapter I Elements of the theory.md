...
## 2. Function Spaces
1. $\mathscr{C}(a,b)$ --- 所有在 $[a,b]$ 上连续的函数，范数为 $\lVert y \rVert _0$
$$
\lVert y \rVert _0 = \max_{a \leq x \leq b}\ \lvert y \rvert
$$
2. $\mathscr{D}_1 (a,b)$ --- 所有在 $[a,b]$ 上有一阶连续导数的函数，范数为 $\lVert y \rVert_1$  
$$
\lVert y \rVert_1 = \max_{a \leq x \leq b}\ \lvert y \rvert + \max_{a \leq x \leq b}\ \lvert y^\prime \rvert
$$
3. $\mathscr{D}_n (a,b)$ --- 所有在 $[a,b]$ 上有 $n$ 阶连续导数的函数，范数为 $\lVert y \rVert_n$
$$
...
> 4. 若 $\alpha(x) \in \mathscr{C}(a,b)$, 且 $\forall h \in \mathscr{D}_1(a,b)$, $h(a)=h(b)=0$, 有
> $$\int_a^b [\alpha(x) h(x) + \beta(x)h^\prime(x)]\, dx = 0$$
> 则 $\exists\, \beta^\prime(x) = \alpha(x),\ \forall x \in [a,b]$ ^lem4


> [!Definition] Variation (Differential) of a Functional
...
> 若
> $$
> \Delta J[h] = \varphi[h] + \varepsilon \lVert h \rVert,\ \varepsilon \to 0\ \text{as}\ \lVert h \rVert \to 0
> $$
> 称 $J[h]$ **可微 (differential)**，称 $\varphi[h]$ 为 $J[y]$ 的 **变分 (variation)**，记为 $\delta J[h]$
...
> 即 $J[y]$ 存在极值的必要条件为：$y$ 满足如上欧拉方程。

如下探究欧拉方程的若干特殊情形
1. $J[y] = \int_a^b F(x,y^\prime)\, dx$ , 欧拉方程为
$$
F_{y^\prime} = C
$$
2. $J[y] = \int_a^b F(y,y^\prime)\, dx$, 欧拉方程为
$$
F - y^\prime F_{y^\prime} = C
$$
3. $J[y] = \int_a^b F(x,y)\, dx$, 欧拉方程为
$$
F_y(x,y) = 0
$$
4. 曲线积分泛函
